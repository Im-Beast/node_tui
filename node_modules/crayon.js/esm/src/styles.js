...
import { ansi4ToAnsi3, ansi8ToAnsi4, hslToRgb, rgbToAnsi8, } from "./conversions.js";
import { colorSupport, styles } from "./crayon.js";
 ...
 * ```
 */
export function hex(value, bg) {
    let hexNum = value;
    if (typeof value !== "number" && (isNaN(hexNum) || value === "")) {
        hexNum = parseInt(value.slice(1), 16);
        if (isNaN(hexNum) || value.slice(1) === "") {
            throw new Error(`Invalid HEX value: "${value}": expected string - "#ABCDEF" / "#abcdef" or number - 0xABCDEF`);
        }
    }
    if (hexNum > 0xFFFFFF || hexNum < 0) {
        throw new Error(`Invalid HEX value: 0x${value.toString(16)}: expected hexadecimal number that doesn't exceed 0xFFFFFF and is higher or equal to zero`);
    }
