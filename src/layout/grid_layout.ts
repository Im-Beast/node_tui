  ...
  updateElements(): void {
    ...
    if (lastElement) {
      ...
      if (lastRowElementNames[0] === bottomRightElementName) {
        ...
        bottomRightRectangle.width -=
          lastElementRectangle.column +
          lastElementRectangle.width -
          width +
          gapX;
      }
      ...
      const piece =
        (diff < 0 ? 1 : -1) * Math.ceil(Math.abs(diff) / elementsByRow.length);
      ...
      for (const element of lastRowElements) {
        const rectangle = element.rectangle.peek();
        rectangle.height +=
          row + height - (rectangle.row + rectangle.height) - gapY;
      }
    }
  }
